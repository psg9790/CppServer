...
public:
	...
	shared_ptr<T> TryPop()
	{
		...
		while (true)
		{
			...
			//최소한 externalCount가 2보다 큰 상태, 삭제X (안전하게 접근할 수 있는 상태)
			...
			if (_head.compare_exchange_strong(oldHead, ptr->next))
			{
				...
				//나 말고 또 누가 있는가?
				const int32 countIncrease = oldHead.externalCount - 2;	//나밖에 없으면 0
				if (ptr->internalCount.fetch_add(countIncrease) == -countIncrease)	//나밖에 없으면 0 == 0
					delete ptr;
